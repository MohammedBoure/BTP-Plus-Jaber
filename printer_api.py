...
# --- إعداد نظام التسجيل (Logging) مع الحذف التلقائي للسجلات الأقدم من شهر ---
log_handler = TimedRotatingFileHandler(
    filename='printer_service.log',
    when='D',            # تدوير الملف يوميًا
    interval=1,          # كل 1 يوم
    backupCount=30,      # احتفظ بآخر 30 ملف (شهر)
    encoding='utf-8'     # لدعم العربية
)
...
def print_receipt(data):
    ...
    try:
        ...
        logger.error(f"خطأ في الاتصال بالطابعة '{PRINTER_NAME}': {e}")
    ...
    try:
        ...
        # --- جدول المنتجات ---
        col_total = MARGIN + 100
        col_price = MARGIN + 230
        col_qty = MARGIN + 350
        col_name = PAGE_WIDTH - MARGIN
        hDC.SelectObject(fonts["normal"])
        hDC.TextOut(col_name, y_pos, "المنتج")
        hDC.SetTextAlign(win32con.TA_CENTER)
        hDC.TextOut(col_qty, y_pos, "الكمية")
        hDC.TextOut(col_price, y_pos, "السعر")
        hDC.SetTextAlign(win32con.TA_LEFT)
        hDC.TextOut(col_total, y_pos, "الإجمالي")
        y_pos += 35
        hDC.SetTextAlign(win32con.TA_RIGHT)
        ...
        if data.get('remiseVente', 0) > 0:
            draw_two_column_text("الخصم", f"{data.get('remiseVente', 0):,.2f} د.ج", fonts["normal"])
        if data.get('fraisLivraison', 0) > 0:
            draw_two_column_text("تكلفة التوصيل", f"{data.get('fraisLivraison', 0):,.2f} د.ج", fonts["normal"])
        if data.get('fraisTravail', 0) > 0:
            draw_two_column_text("تكلفة العمال", f"{data.get('fraisTravail', 0):,.2f} د.ج", fonts["normal"])
        ...
        if data.get('estCredit'):
            draw_two_column_text("المتبقي", f"{data.get('resteAPayer', 0):,.2f} د.ج", fonts["normal"])
        draw_line_separator()
...
def handle_print_request():
    ...
    logger.info(json.dumps(receipt_data, indent=2, ensure_ascii=False))
    
    try:
        print_receipt(receipt_data)
        logger.info("تم إرسال الفاتورة إلى الطابعة بنجاح.")
...
if __name__ == '__main__':
    logger.info("خادم الطباعة بدأ العمل على http://127.0.0.1:5000")
